...
#
# The point of this class is to simply ping the available Homeway server regions occasionally to track which region is has the best
...
class PingPong:
    ...
    def Init(logger, pluginDataFolderPath):
        PingPong._Instance = PingPong(logger, pluginDataFolderPath)
    ...
    def __init__(self, logger:logging.Logger, pluginDataFolderPath:str):
        self.Logger = logger
    ...
    def GetLowestLatencyServerSub(self):
        ...
        try:
            ...
            if stats is None:
                return None
            if PingPong.LowestLatencyServerSubKey not in stats:
                return None
            lowestLatencyServerSub = stats[PingPong.LowestLatencyServerSubKey]
    ...
    def _WorkerThread(self):
        ...
        while True:
            try:
                ...
                lastWorkTime = 0
                if PingPong.LastWorkTimeKey in self.Stats:
                    lastWorkTime = int(self.Stats[PingPong.LastWorkTimeKey])
                ...
                if lastWorkTime == 0:
                    ...
                    # Since the first run will be a little after device boot, we need to wait a bit before for things to settle.
                ...
                # which might reconnect the main Socket connection to the best possible server.
    ...
    def _ComputeStats(self, defaultServerResult):
        ...
        #
        # Note, we should be mindful that if the plugin is not connected to the default server, there's a higher chance that an app connection
        ...
        try:
            ...
            with requests.Session() as s:
                ...
                if "Result" not in obj:
                    self.Logger.warn("PingPong server response had no result obj.")
                    return None
                if "Servers" not in obj["Result"]:
                    self.Logger.warn("PingPong server response had no servers obj.")
                    return None
                servers = obj["Result"]["Servers"]
                if "ThisServer" not in obj["Result"]:
                    self.Logger.warn("PingPong server response had no ThisServer obj.")
                    return None
                thisServer = obj["Result"]["ThisServer"]
                if "EnablePluginAutoLowestLatency" not in obj["Result"]:
                    self.Logger.warn("PingPong server response had no EnablePluginAutoLowestLatency obj.")
                    return None
