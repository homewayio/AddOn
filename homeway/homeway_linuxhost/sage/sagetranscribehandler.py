...
class SageTranscribeHandler:
    ...
    def __init__(self, logger:logging.Logger, sageHandler, fiberManager:FiberManager, startAudioEvent:Event):
        self.Logger = logger
   ...
   # Handles all streaming audio for speech to text.
    async def HandleStreamingAudio(self, event: Event) -> bool:
        ...
        if AudioChunk.is_type(event.type):
            ...
            start = time.time()
            result = await self.FiberManager.Listen(False, self.Buffer, SageDataTypesFormats.AudioPCM, self.IncomingAudioStartEvent.rate, self.IncomingAudioStartEvent.channels, self.IncomingAudioStartEvent.width)
